...
- **Draft → version flow**: editing a saved blueprint autosaves a *draft* (`draft_sql` / `draft_positions` columns on `blueprints`). "Save as new version" commits an immutable `blueprint_versions` row. "Restore" copies an old version forward into a new one (history is never destroyed).
